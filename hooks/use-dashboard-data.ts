  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filtrar por turma atual se houver uma selecionada
        if (currentClass) {
          // Em uma implementação real, você filtraria por turma no backend
          // Aqui estamos simulando o filtro
          assessments = assessments.filter(a => a.id.includes(currentClass.id.charAt(currentClass.id.length - 1)))
        }
        ...
        const averageGrade =
          totalAssessments > 0 ? Math.round(filteredAssessments.reduce((sum, a) => sum + a.grade, 0) / totalAssessments) : 0
